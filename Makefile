.PHONY: help build run test clean fmt lint lint-fix sec vet tidy deps install dev hot check coverage b r t c f l lf s v check-deps templ templ-watch
...
help:
	...
	@echo "  make sec (s)        - Run security scanners (gosec + govulncheck)"
...
fmt f:
	...
	@gofmt -s -w .
	@command -v goimports >/dev/null 2>&1 && goimports -w -local github.com/xraph/herald . || echo "$(YELLOW)goimports not found, skipping (run: go install golang.org/x/tools/cmd/goimports@latest)$(NC)"
...
## sec (s): Run security scanners
sec s:
	@echo "$(BLUE)Running security scanners...$(NC)"
	@command -v gosec >/dev/null 2>&1 || { echo "$(YELLOW)gosec not found, installing...$(NC)"; go install github.com/securego/gosec/v2/cmd/gosec@latest; }
	gosec ./...
	@command -v govulncheck >/dev/null 2>&1 || { echo "$(YELLOW)govulncheck not found, installing...$(NC)"; go install golang.org/x/vuln/cmd/govulncheck@latest; }
	govulncheck ./...
