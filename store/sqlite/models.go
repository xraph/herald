...
func toProviderModel(p *provider.Provider) *providerModel {
	creds, _ := json.Marshal(p.Credentials) //nolint:errcheck // best-effort
	settings, _ := json.Marshal(p.Settings) //nolint:errcheck // best-effort
...
func fromProviderModel(m *providerModel) (*provider.Provider, error) {
	...
	if m.Credentials != "" {
		_ = json.Unmarshal([]byte(m.Credentials), &creds) //nolint:errcheck // best-effort
	}
	var settings map[string]string
	if m.Settings != "" {
		_ = json.Unmarshal([]byte(m.Settings), &settings) //nolint:errcheck // best-effort
	}
...
type templateModel struct {
	...
	Variables string    `grove:"variables"` // JSON text
...
func fromTemplateModel(m *templateModel) (*template.Template, error) {
	...
	if m.Variables != "" {
		_ = json.Unmarshal([]byte(m.Variables), &vars) //nolint:errcheck // best-effort
	}
...
func toNotificationModel(n *inbox.Notification) *notificationModel {
	metadata, _ := json.Marshal(n.Metadata) //nolint:errcheck // best-effort
...
type preferenceModel struct {
	...
	AppID     string    `grove:"app_id"`
	UserID    string    `grove:"user_id"`
	Overrides string    `grove:"overrides"` // JSON text
	CreatedAt time.Time `grove:"created_at"`
	UpdatedAt time.Time `grove:"updated_at"`
}
...
func fromPreferenceModel(m *preferenceModel) (*preference.Preference, error) {
	...
	if m.Overrides != "" {
		_ = json.Unmarshal([]byte(m.Overrides), &overrides) //nolint:errcheck // best-effort
	}
